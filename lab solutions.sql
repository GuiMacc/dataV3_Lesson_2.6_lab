...
SELECT count(distinct rental_id) AS 'nº of rentals', staff_id FROM rental
GROUP BY staff_id;

-- 4. Using the film table, find out how many films were released each year.

SELECT count(distinct film_id) AS 'amount of films released', release_year FROM film
GROUP BY release_year;

-- 5. Using the film table, find out for each rating how many films were there.

SELECT count(distinct film_id) AS 'amount of films released', rating FROM film
GROUP BY rating;
...
WHERE (length IS NOT NULL) AND NOT (length = '  ') AND NOT (length='');